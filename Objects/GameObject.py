...
class Agobject(S_MonoBehaviour):
    ...
    def Update(self):
        if self.GameObject[-1].ivents['strong share']:
            ...
            self.AnimU()
            for i in self.Input:
                if i == 'a' and self.Input[i]:
                    self.transform['x'] -= self.WalkSpeed if self.transform['x'] > 32.5 else 0
                if i == 'd' and self.Input[i]:
                    self.transform['x'] += self.WalkSpeed if self.transform['x'] < 437.5 else 0
                if i == 'w' and self.Input[i]:
                    self.transform['y'] -= self.WalkSpeed if self.transform['y'] > 0 else 0
                if i == 's' and self.Input[i]:
                    self.transform['y'] += self.WalkSpeed if self.transform['y'] < 390 else 0
        ...
        if self.feel['anim'] != 'Idle' and self.feel['frame'] == self.anims[self.feel['anim']][0] - 1:
            ...
            self.feel['frame'] = 0
            self.AnimU()
        self.feel['frame'] += self.anims[self.feel['anim']][2] if self.feel['frame'] < self.anims[self.feel['anim']][
            0] - 1 else - self.anims[self.feel['anim']][0] + 1
